...
async def search_food(textsearch: str, response: Response, request: Request):
    ...
    if not foods:
        ...
        result = f'''Can't find "{textsearch}"'''
    else:
...
async def getDetailFoodById(request: Request, respose: Response, fid: str):
    ...
    reviews = reviewsEntity(conn.foodsell.reviews.find({"f_id": fid}))
    access_token = request.cookies.get('access_token')
    user = None
    if access_token != None and auth.checkJWTExpired(access_token):
        user = auth.get_current_user(access_token)
        return templates.TemplateResponse("detail.html", {"request":request, "food":food, "reviews": reviews, "current_user":user})
